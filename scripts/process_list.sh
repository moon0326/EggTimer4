...
if [ $1 = newtimer ]; then
  osascript -e 'tell application "Alfred" to search "timer "'
  exit
fi
#"Create New Alarm" option selected
if [ $1 = newalarm ]; then
  osascript -e 'tell application "Alfred" to search "alarm "'
  exit
fi
...
if [ $1 = recent ]; then
  ...
  osascript -e "tell application \"Alfred\" to search \"$keyword\""
  exit
fi
